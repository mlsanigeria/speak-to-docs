...
st.set_page_config(page_title="Speak-To-Docs", page_icon="📝",
...
def get_llm() -> AzureChatOpenAI:
    try:
        ...
        st.error(
            "An error occurred while initializing the language model. Please try again later.")
        return None
    ...
    try:
        ...
        for file_path in extracted_file_paths:
            try:
                ...
                logger.info(
                    f"Document {file_path} chunked into {len(chunks)} chunks.")
            except Exception as e:
                logger.error(
                    f"Error reading or chunking file '{file_path}': {e}")
                continue

        # initializing the vector store
        vector_store = DocArrayInMemorySearch.from_documents(
            docs, openai_embeddings)
        logger.info(
            "DocArrayInMemorySearch vector store initialized successfully.")

        return vector_store

    except Exception as e:
        logger.exception(
            f"An error occurred while initializing the vector store: {e}")
...
with st.sidebar:
        ...
        if len(st.session_state.uploaded_files) > 2:
            ...
            for file in st.session_state.uploaded_files:
                if allowed_files(file.name):
                    ...
                    if num_pages > 50:
                        st.error(
                            f"{file.name} exceeds the 50-page limit (has {num_pages} pages).")
                        logging.warning(
                            f"File {file.name} exceeds the page limit.")
                        valid_file = False
                        break
                    else:
            ...
            if valid_file and valid_files:
                try:
                    extraction_results = extract_contents_from_doc(
                        valid_files, "temp_dir")
                    vector_store = create_vector_store(extraction_results)
                    if vector_store:
                        st.session_state['vector_store'] = vector_store
                        st.success(
                            f"{len(st.session_state.uploaded_files)} file(s) uploaded and processed successfully.")
                        logging.info(
                    ...
                    logging.error(
                        f"Error extracting content from document: {e}")
    else:
        st.session_state.uploaded_files = None
    ...
    if 'speech_outputs' in st.session_state:
        for speech_output in st.session_state.speech_outputs:
            st.audio(os.path.join('speech_outputs', speech_output),
                     format="audio/wav", start_time=0)


def send_response(message, response=None):
    response = create_chat_completion(llm, message)
...
def handle_audio_message():
    ...
    try:
        ...
        if speech_text:
            ...
            send_response(
                speech_text, "Sorry, I couldn't transcribe your audio. Please try again.")
...
audio_value = st.experimental_audio_input(
    "Record a voice message", key="audio_prompt", on_change=handle_audio_message)
